...
5. После загрузки проверьте, что в корне есть `.htaccess` (заголовки безопасности). Если панель скрывает «точки» — включите показ скрытых файлов.

**Важно:** пароль FTP, если когда‑либо светился в чате — сразу смените в ispmanager. Не храните пароль в репозитории.
